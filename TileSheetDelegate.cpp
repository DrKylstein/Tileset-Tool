...
#include <QtGui>
TileSheetDelegate::TileSheetDelegate(QWidget* parent): QAbstractItemDelegate(parent) {
}
void TileSheetDelegate::paint (QPainter* painter, const QStyleOptionViewItem& option, const QModelIndex& index) const {
	if(index.data().value<QPixmap>().isNull()) {
...
QSize TileSheetDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const {
	return QSize();
}
